...
func TestAnnotsRoundTripThroughTheTodo(t *testing.T) {
	want := annots{Priority: priorityCritical, Fruit: true, Value: valueHigh, Flag: true, FlagNote: "why"}
	var td Todo
	want.applyTo(&td)
...
func TestAnnotsSummaryNamesOnlyWhatWasSaid(t *testing.T) {
	cases := []struct {
		...
		{annots{Value: valueHigh}, "high value"},
		...
		{annots{Priority: priorityCritical, Fruit: true, Value: valueHigh},
...
func TestAnnotationsStayOutOfTheJSONWhenUnset(t *testing.T) {
	...
	if err := s.setAnnots("a1", annots{Fruit: true, Value: valueHigh}); err != nil {
		t.Fatal(err)
	}
...
func TestSetAnnotsWritesTheWholeSetAtOnce(t *testing.T) {
	...
	if err := s.setAnnots("a1", annots{Priority: priorityCritical, Value: valueHigh}); err != nil {
		t.Fatal(err)
	}
	td, _ := s.find("a1")
	if td.Priority != priorityCritical || td.Fruit || td.valueLevel() != valueHigh {
...
func TestEverySlotDrawsADistinctGlyph(t *testing.T) {
	...
	all := Todo{ID: "x", Prompt: "p", Priority: priorityCritical, Fruit: true, HighValue: true, Flag: true, Info: true}
	...
	for _, sl := range annotSlots {
		for _, td := range []Todo{
			...
			{HighValue: true}, {Value: valueMedium}, {Flag: true}, {Info: true},
		} {
